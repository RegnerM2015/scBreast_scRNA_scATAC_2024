...
idxSample <- BiocGenerics::which(proj$predictedGroup %in% c("20",
                                                            "6",
                                                            "23",
                                                            "18",
                                                            "12",
...
proj$Keep.Cells <- ifelse(proj$predictedGroup == "9" & proj$From_Normal_Sample == "No","Drop","Keep" )
...
proj$From_Luminal_Sample <- ifelse(proj$Sample %in% luminals,"Yes","No")

proj$Keep.Cells <- ifelse(proj$predictedGroup %in% c("20",
                                                     "6",
                                                     "23",
                                                     "18",
                                                     "12") & proj$From_Luminal_Sample == "No","Drop","Keep" )
...
proj$Keep.Cells <- ifelse(meta$predictedGroup %in% c("20",
                                                     "6",
                                                     "23",
                                                     "18",
                                                     "12") & meta$SCSubtype %ni% c("LumA_SC","LumB_SC"),"Drop","Keep" )
...
sub <- df[df$predictedGroup == "9",]
table(sub$Sample)

sub <- df[df$predictedGroup %in% c("20",
                                   "6",
                                   "23",
                                   "18",
                                   "12"),]
...
subsetArchRProject(
  ArchRProj = proj,
  cells = cellsSample,
  outputDirectory = "Luminal_TN_Samples_scATAC-TESTING3",
  dropCells = FALSE,
  logFile = NULL,
