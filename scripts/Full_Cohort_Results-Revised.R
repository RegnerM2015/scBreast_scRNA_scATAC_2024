...
# Make output folder
if(dir.exists("Full_Cohort_Results-Revised")){
  print("Directory Full_Cohort_Results-Revised already exists!")
}else{
  dir.create("Full_Cohort_Results-Revised")
}
...
ggsave("./Full_Cohort_Results-Revised/UMAP_RNA_Cluster_Number.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_RNA_Cluster_Number-Labels.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_RNA_Cell_Type_Cluster.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_RNA_Cell_Type_Cluster-Labels.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_ATAC_Cluster_Number.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_ATAC_Cluster_Number-Labels.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_RNA_Patient_Sample.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_RNA_Patient_Sample-Labels.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_ATAC_Patient_Sample.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_ATAC_Patient_Sample-Labels.pdf",width = 8,height = 7)
...
p1+p2+p3
ggsave(paste0("./Full_Cohort_Results-Revised/Proportion_Bar_Charts_Sample_inferCNV_SCSubtype_RNA",".pdf"),
       width = 14,
       height = 7)
...
p4 <- df %>%
  ...
  ggtitle("scRNA-seq")
p4
ggsave(paste0("./Full_Cohort_Results-Revised/Proportion_Bar_Charts_Reference_Cell_Type_Annotations_RNA",".pdf"),
       width = 10,
       height = 7)
...
p1+p2+p3
ggsave(paste0("./Full_Cohort_Results-Revised/Proportion_Bar_Charts_Sample_inferCNV_SCSubtype_ATAC",".pdf"),
       width = 14,
       height = 7)
...
ggsave("./Full_Cohort_Results-Revised/QC_Histograms_RNA_ATAC.pdf",width = 9,height = 8)
...
df$Sample <- factor(df$Sample,levels=rev(c("49758L",
                                           "49CFCL",
                                           "4AF75L",
                                           "4B146L",
                                           "35A4AL",
...
ggsave("./Full_Cohort_Results-Revised/QC_Boxplots_RNA_ATAC.pdf",width = 9,height = 8)
...
ggsave("./Full_Cohort_Results-Revised/UMAP_ATAC_Prediction_Scores.pdf",width = 8,height = 7)
...
ggsave("./Full_Cohort_Results-Revised/Histogram_ATAC_Prediction_Scores.pdf",width = 8,height = 12)
...
ggsave(paste0("./Full_Cohort_Results-Revised/Proportion_Bar_Charts_Cell_Type_inferCNV_Subtype",".pdf"),
       width = 28,
       height = 10)
...
meta <- as.data.frame(rna@meta.data)

meta$Sample <- factor(meta$orig.ident,levels=rev(c("49758L",
                                                   "49CFCL",
                                                   "4AF75L",
                                                   "4B146L",
                                                   "35A4AL",
...
df <- meta %>% dplyr::group_by_at("Sample") %>% dplyr::count(SCSubtype)
colnames(df) <- c("Sample","SCSubtype","Cells")
pSubtypeBySample.RNA <- df %>%
  ggplot(aes(fill=SCSubtype, y=Cells, x= Sample))+
  geom_bar(position="fill", stat="identity")+NoLegend()+
  theme(axis.title.x = element_text(size = 9))+
  theme(axis.text.x = element_text(size = 8))+
  theme(axis.title.y = element_text(size = 9))+
  theme(axis.text.y = element_text(size = 8))+
  coord_flip()+theme_classic()+xlab("Sample")+ylab("Fraction of cells")+
  ggtitle(paste0("scRNA-seq: ",nrow(meta)," cells"))

df.tmp <- meta[meta$SCSubtype %in% c("LumA_SC",
                                     "Basal_SC",
                                     "LumB_SC",
                                     "Her2E_SC"),]
df <- df.tmp %>% dplyr::group_by_at("Sample") %>% dplyr::count(SCSubtype)
colnames(df) <- c("Sample","SCSubtype","Cells")
pSubtypeBySampleCancerOnly.RNA <- df %>%
  ggplot(aes(fill=SCSubtype, y=Cells, x= Sample))+
  geom_bar(position="fill", stat="identity")+NoLegend()+
  theme(axis.title.x = element_text(size = 9))+
  theme(axis.text.x = element_text(size = 8))+
  ...
  theme(axis.text.y = element_text(size = 8))+
  coord_flip()+theme_classic()+xlab("Sample")+ylab("Fraction of cells")+
  ggtitle(paste0("scRNA-seq: ",nrow(df.tmp)," cells"))
...
meta$Sample <- factor(meta$Sample,levels=rev(c("49758L",
                                           "49CFCL",
                                           "4AF75L",
                                           "4B146L",
                                           "35A4AL",
...
pSubtypeBySample.ATAC <- df %>%
  ...
  theme(axis.text.y = element_text(size = 8))+
  coord_flip()+theme_classic()+xlab("Sample")+ylab("Fraction of cells")+
  ggtitle(paste0("scATAC-seq: ",nrow(meta)," cells"))
...
df <- df.tmp %>% dplyr::group_by_at("Sample") %>% dplyr::count(SCSubtype)
colnames(df) <- c("Sample","SCSubtype","Cells")
pSubtypeBySampleCancerOnly.ATAC <- df %>%
  ggplot(aes(fill=SCSubtype, y=Cells, x= Sample))+
  geom_bar(position="fill", stat="identity")+NoLegend()+
  theme(axis.title.x = element_text(size = 9))+
  theme(axis.text.x = element_text(size = 8))+
  ...
  theme(axis.text.y = element_text(size = 8))+
  coord_flip()+theme_classic()+xlab("Sample")+ylab("Fraction of cells")+
  ggtitle(paste0("scATAC-seq: ",nrow(df.tmp)," cells"))


pSubtypeBySample.RNA+pSubtypeBySampleCancerOnly.RNA+pSubtypeBySample.ATAC+pSubtypeBySampleCancerOnly.ATAC+plot_layout(ncol = 2)
ggsave(paste0("./Full_Cohort_Results-Revised/Proportion_Bar_Charts_SCSubtype_RNA_and_ATAC",".pdf"),
       width = 9,height = 8)
...
pdf("./Full_Cohort_Results-Revised/Pseudobulk_Hierarchical_Clustering_Dendrogram.pdf",width=8,height = 9)
res[[1]]
dev.off()

pdf("./Full_Cohort_Results-Revised/Pseudobulk_Hierarchical_Clustering_Heatmap.pdf",width=10,height = 8)
res[[2]]
dev.off()

saveRDS(res[[3]],"./Full_Cohort_Results-Revised/rlog_object.rds")

saveRDS(res[[4]],"./Full_Cohort_Results-Revised/rlog_object-top_variable_features.rds")
...
ggsave("./Full_Cohort_Results-Revised/PCA_cellType_status_cluster_sample.pdf",
       width = 11,
       height = 11)
...
p1+p2+p3+p4+plot_layout(ncol = 2)
ggsave("./Full_Cohort_Results-Revised/PCA_cellCount_sizeFactor_BMI_Age.pdf",
       width = 11,
       height = 11)
