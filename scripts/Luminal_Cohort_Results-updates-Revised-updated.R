...
# Make output folder
if(dir.exists("Luminal_Cohort_Results-updates-Revised-updated")){
  print("Directory Luminal_Cohort_Results-updates-Revised-updated already exists!")
}else{
  dir.create("Luminal_Cohort_Results-updates-Revised-updated")
}
...
# Plot seurat UMAP
seurat <- readRDS("Processed_Seurat_Objects_scRNA/Processed_Seurat_Object_Luminal_TN_Subset-TESTING.rds")
...
cols <- c("#828282","#a36d6d","#a66262","#ab5454","#a84343","#a63333","#a12222","#9c1414")
...
df$cellType <- ifelse(df$cellType == "0","Normal_ML",df$cellType)
df$cellType <- ifelse(df$cellType == "1","Luminal_BC_44F0AL",df$cellType)
df$cellType <- ifelse(df$cellType == "2","Luminal_BC_3D388L",df$cellType)
df$cellType <- ifelse(df$cellType == "3","Luminal_BC_3C7D1L_3821AL_35EE8L",df$cellType)
df$cellType <- ifelse(df$cellType == "4","Luminal_BC_43E7BL_43E7CL",df$cellType)
df$cellType <- ifelse(df$cellType == "5","Luminal_BC_3B3E9L",df$cellType)
df$cellType <- ifelse(df$cellType == "6","Luminal_BC_3FCDEL",df$cellType)
df$cellType <- ifelse(df$cellType == "7","Luminal_BC_45CB0L",df$cellType)
...
# Plot UMAP by sample
cols <- c("#B2DF8A", "#33A02C", "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#a97ac2", "#ccb4d9", "#6A3D9A", "#B15928", "#B4B4B4", "#828282", "#505050", "#323232")

df <- as.data.frame(seurat@reductions$umap@cell.embeddings)
all.equal(rownames(df),rownames(seurat@meta.data))
df$sample <- factor(seurat$orig.ident,levels=c("35EE8L",
                                               "3821AL",
                                               "3B3E9L",
                                               "3C7D1L",
                                               "3D388L",
...
df$sample <- factor(df$sample,levels=c("35EE8L",
                                       "3821AL",
                                       "3B3E9L",
                                       "3C7D1L",
                                       "3D388L",
...
ggsave(paste0("./Luminal_Cohort_Results-updates-Revised-updated/UMAPs_",
              "Luminal_Cell_Type_Cluster",".pdf"),
       width = 14,
       height = 6 )
...
seurat <- readRDS("Processed_Seurat_Objects_scRNA/Processed_Seurat_Object_Luminal_TN_Subset-TESTING.rds")
seurat$cancer <- ifelse(seurat$orig.ident %in% c("49758L","49CFCL", "4AF75L", "4B146L"),"normal","cancer")
seurat$patient <- ifelse(seurat$orig.ident %in% c("43E7CL","43E7BL"),"43E7BL|43E7CL",seurat$orig.ident)
seurat$full <- "full"
seurat <- seurat[,seurat$orig.ident != "3B3E9L"]
...
clinical$sample_id <- ifelse(clinical$Sample %in% c("43E7CL","43E7BL"),"43E7BL|43E7CL",clinical$Sample)
...
ggsave(
  filename = paste0("./Luminal_Cohort_Results-updates-Revised-updated/",
...
# Write out dds
saveRDS(dds,"./Luminal_Cohort_Results-updates-Revised-updated/pseudobulk_expression_dds.rds")
...
pdf("Luminal_Cohort_Results-updates-Revised-updated/EnhancedVolcano_DEGs.pdf",width=10,height=10)
EnhancedVolcano(res,
                lab = rownames(res),
                x = 'log2FoldChange',
                y = 'padj',
                title = 'Luminal-like cancer versus normal ML',
                pCutoff = 0.05,
                FCcutoff = 0.58,
                pointSize = 3.0,
                labSize = 6.0)
...
saveRDS(degs,paste0("./Luminal_Cohort_Results-updates-Revised-updated/DEGs_padj_",padj_cutoff,"_log2FC_",log2fc_cutoff,".rds"))

## Extract normalized counts from dds object
normalized_counts <- counts(dds, normalized = TRUE)
saveRDS(normalized_counts,"./Luminal_Cohort_Results-updates-Revised-updated/pseudobulk_expression_normalized_counts.rds")
...
ggsave(paste0("./Luminal_Cohort_Results-updates-Revised-updated/DEGs_",
              "Luminal",".pdf"),
       width = 5,
       height = 5 )
...
saveRDS(up.genes,"./Luminal_Cohort_Results-updates-Revised-updated/up_genes.rds")
...
pdf("Luminal_Cohort_Results-updates-Revised-updated/GSEA_Luminal_DEGs.pdf",width=10,height=4)
...
saveRDS(resSig,"./Luminal_Cohort_Results-updates-Revised-updated/resSig_genes.rds")
...
pdf("Luminal_Cohort_Results-updates-Revised-updated/Heatmap_of_DEGs_Luminal.pdf",width=6,height=4)
...
idxSample <- BiocGenerics::which(proj$predictedGroup %in% samplesToUse)
...
metadata$full <- "full"
metadata$patient <- ifelse(metadata$Sample %in% c("43E7BL","43E7CL"),"43E7BL|43E7CL",metadata$Sample)
# Create single cell experiment object
...
# Write out dds
saveRDS(dds,"./Luminal_Cohort_Results-updates-Revised-updated/pseudobulk_accessibility_dds.rds")
...
pdf("Luminal_Cohort_Results-updates-Revised-updated/EnhancedVolcano_DAPs.pdf",width=10,height=10)
EnhancedVolcano(res,
                lab = rownames(res),
                x = 'log2FoldChange',
                y = 'padj',
...
saveRDS(daps,paste0("./Luminal_Cohort_Results-updates-Revised-updated/DAPs_padj_",padj_cutoff,"_log2FC_",log2fc_cutoff,".rds"))

## Extract normalized counts from dds object
normalized_counts <- counts(dds, normalized = TRUE)
saveRDS(normalized_counts,"./Luminal_Cohort_Results-updates-Revised-updated/pseudobulk_accessibility_normalized_counts.rds")
...
ggsave(paste0("./Luminal_Cohort_Results-updates-Revised-updated/DAPs_",
              "Luminal",".pdf"),
       width = 5,
       height = 5 )
...
saveRDS(up.peaks,"./Luminal_Cohort_Results-updates-Revised-updated/up_peaks.rds")

# Plot heatmap of degs using rlog values

resSig = res[ which(res$padj < 0.05 & abs(res$log2FoldChange) > 0.58), ]
saveRDS(resSig,"./Luminal_Cohort_Results-updates-Revised-updated/resSig_peaks.rds")
allSig_genes <- rownames(resSig)
...
pdf("Luminal_Cohort_Results-updates-Revised-updated/Heatmap_of_DAPs_Luminal.pdf",width=6,height=4)
ComplexHeatmap::Heatmap(t(scale(t(mat[allSig_genes,]))),cluster_rows = T,cluster_columns = TRUE)
dev.off()
...
up.peaks <- readRDS("./Luminal_Cohort_Results-updates-Revised-updated/up_peaks.rds")
...
# Unbalanced
meta <- fread("LME_Luminal_out-SingFits_OLS/interactionLMM_univariateLMM_and_OLS_results-metacells.csv")
...
saveRDS(p2g,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_univariate-prefilter.rds")
...
saveRDS(p2g,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_univariate-postfilter.rds")
...
ggsave("./Luminal_Cohort_Results-updates-Revised-updated/peakType_proportionBarchart-univariate.pdf",width = 4, height = 4)
...
ggsave("./Luminal_Cohort_Results-updates-Revised-updated/encode_proportionBarchart-univariate.pdf",width = 4,height = 4)
...
pdf("Luminal_Cohort_Results-updates-Revised-updated/peakType_and_ENCODE_overlap_proportion_bar_charts.pdf",width=7,height=4)
cowplot::plot_grid(peakTypePropUni,encodePropUni, ncol = 2, nrow = 1)
dev.off()

pdf("Luminal_Cohort_Results-updates-Revised-updated/peakType_and_ENCODE_overlap_proportion_bar_charts-NoLegend.pdf",width=5,height=4)
...
saveRDS(p2g,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction.rds")
...
ggsave("./Luminal_Cohort_Results-updates-Revised-updated/peakType_proportionBarchart-interaction.pdf",width = 4, height = 4)
...
ggsave("./Luminal_Cohort_Results-updates-Revised-updated/encode_proportionBarchart-interaction.pdf",width = 4,height = 4)
...
DiffClassBar <- ggplot(df,aes(x=DiffClass,y=n))+
  geom_bar(stat='identity',
           fill=c(colsToUse[1],colsToUse[4],
                  ...
                  colsToUse[6],colsToUse[7],
                  colsToUse[5],colsToUse[8]))+
...
pdf("Luminal_Cohort_Results-updates-Revised-updated/p2g_scatter_plots_and_DiffClass_bar_chart.pdf",width=18,height=6)
cowplot::plot_grid(p2gScatterBoth, p2gScatterDiffClass, DiffClassBar, ncol = 3, nrow = 1)
dev.off()

pdf("Luminal_Cohort_Results-updates-Revised-updated/p2g_scatter_plots_and_DiffClass_bar_chart-NoLegend.pdf",width=14,height=6)
cowplot::plot_grid( 
...
p2gScatterDiffClassFacet <-  p2g %>% 
  ...
  scale_color_manual(values=c(colsToUse[1],colsToUse[4],
                              ...
                              colsToUse[5],colsToUse[8]))+
  ggtitle(paste(nrow(p2g)," significant differential P2Gs"))
ggsave("Luminal_Cohort_Results-updates-Revised-updated/p2g_scatter_plots-FacetByDiffClass.pdf",width=8,height=8)
...
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs-Heatmap.pdf",width=8,height=16)
...
plotDiffClassPlots <- function(p2g,
  ...
  if(!is.null(up.genes)){
    if(is.null(up.peaks)){
      pdf(paste0(path,"/Significant_Differential_P2Gs-Heatmap_",gsub(pattern="/",replacement = "_",x=DiffClass),"_upgenes.pdf"),width=4.15,height=4.25)
      
    }else{
      pdf(paste0(path,"/Significant_Differential_P2Gs-Heatmap_",gsub(pattern="/",replacement = "_",x=DiffClass),"_upgenes_uppeaks.pdf"),width=4.15,height=4.25)
    }
  }else{
    pdf(paste0(path,"/Significant_Differential_P2Gs-Heatmap_",gsub(pattern="/",replacement = "_",x=DiffClass),".pdf"),width=4.15,height=4.25)
  }
  ...
  if(!is.null(up.genes)){
    ...
    if(!is.null(up.genes)){
      if(is.null(up.peaks)){
        pdf(paste0(path,"/Significant_Differential_P2Gs-Heatmap_",gsub(pattern="/",replacement = "_",x=DiffClass),"_upgenes-rankedByLog2FC.pdf"),width=4.15,height=4.25)
        
      }else{
        pdf(paste0(path,"/Significant_Differential_P2Gs-Heatmap_",gsub(pattern="/",replacement = "_",x=DiffClass),"_upgenes_uppeaks-rankedByLog2FC.pdf"),width=4.15,height=4.25)
      }
    }else{
      pdf(paste0(path,"/Significant_Differential_P2Gs-Heatmap_",gsub(pattern="/",replacement = "_",x=DiffClass),"-rankedByLog2FC.pdf"),width=4.15,height=4.25)
    }
...
plotDiffClassPlots(p2g=p2g,
                   ...
                   up.genes=up.genes,
                   up.peaks=NULL,
                   geneOfInterest = "CRABP2",
                   brca.gr=brca.gr,
                   encode.gr=encode.gr,
                   seed=123,
                   path="Luminal_Cohort_Results-updates-Revised-updated")
...
degs <- readRDS(paste0("./Luminal_Cohort_Results-updates-Revised-updated/DEGs_padj_",padj_cutoff,"_log2FC_",log2fc_cutoff,".rds"))
...
# 
# saveRDS(p2g,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-+_0.rds")
# 
# # Screen for up genes
# p2g.up.gene <- p2g[p2g$LME_cell_type_1_model_geneName %in% up.genes$gene,]
# 
# saveRDS(p2g.up.gene,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-+_0-upgenes.rds")
# 
# # Annotate ENCODE and TCGA regions
# 
# # Downloaded TCGA BRCA peaks from https://gdc.cancer.gov/about-data/publications/ATACseq-AWG 
...
#                      c("#CCCCFF", "white", "red"))
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs-Heatmap_+_0_upgenes.pdf",width=4.15,height=4.25)
# ht <- ComplexHeatmap::Heatmap(mat, 
#                               name = "beta",
#                               col = col_fun, 
#                               right_annotation = ha, 
...
# 
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs_+_0_upgenes.pdf",width=18,height=4)
# p1+p2+p3
# dev.off()
# 
# # Gene set ORA 
...
#                                             qvalueCutoff=1)
# 
# saveRDS(em.p2g.up.gene,"./Luminal_Cohort_Results-updates-Revised-updated/em_p2g_+_0_up_gene.rds")
# 
...
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_+_0_upgenes-DotPlot.pdf", 
#        width = 11,
#        height = 6)
# 
# barplot(em.p2g.up.gene,showCategory=3)+
#   ggtitle(paste0(length(unique(p2g.up.gene$LME_cell_type_1_model_geneName))," unique genes"))
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_+_0_upgenes-BarPlot_Count.pdf", 
#        width = 11,
#        height = 6)
# 
# mutate(em.p2g.up.gene, qscore = -log(p.adjust, base=10)) %>% 
#   barplot(x="qscore",showCategory=3)
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_+_0_upgenes-BarPlot_QScore.pdf", 
#        width = 11,
#        height = 6)
# 
...
#                              p2g$LME_cell_type_1_model_peakName %in% up.peaks$gene,]
# 
# saveRDS(p2g.up.gene.up.peak,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-+_0-upgenes_uppeaks.rds")
# 
# # Annotate ENCODE and TCGA regions
# 
# # Downloaded TCGA BRCA peaks from https://gdc.cancer.gov/about-data/publications/ATACseq-AWG 
...
#                      c("#CCCCFF", "white", "red"))
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs-Heatmap_+_0_upgenes_uppeaks.pdf",width=4.15,height=4.25)
# ht <- ComplexHeatmap::Heatmap(mat, 
#                               name = "beta",
#                               col = col_fun, 
#                               right_annotation = ha, 
...
# 
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs_+_0_upgenes_uppeaks.pdf",width=18,height=6)
# p1+p2+p3
# dev.off()
# 
# # Gene set ORA 
...
#                                                     qvalueCutoff=1)
# 
# saveRDS(em.p2g.up.gene.up.peak,"./Luminal_Cohort_Results-updates-Revised-updated/em_p2g_+_0_up_gene_up_peak.rds")
# 
...
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_+_0_upgenes_uppeaks-DotPlot.pdf", 
#        width = 11,
#        height = 6)
# 
# barplot(em.p2g.up.gene.up.peak,showCategory=3)+
#   ggtitle(paste0(length(unique(p2g.up.gene.up.peak$LME_cell_type_1_model_geneName))," unique genes"))
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_+_0_upgenes_uppeaks-BarPlot_Count.pdf", 
#        width = 11,
#        height = 6)
# 
# mutate(em.p2g.up.gene.up.peak, qscore = -log(p.adjust, base=10)) %>% 
#   barplot(x="qscore",showCategory=3)
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_+_0_upgenes_uppeaks-BarPlot_QScore.pdf", 
#        width = 11,
#        height = 6)
# 
# 
...
# p2g <- p2g[p2g$DiffClass == "0/+",]
# 
# saveRDS(p2g,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-0_+.rds")
# 
...
# p2g.up.gene <- p2g[p2g$LME_cell_type_1_model_geneName %in% up.genes$gene,]
# 
# saveRDS(p2g.up.gene,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-0_+-upgenes.rds")
# 
# # Annotate ENCODE and TCGA regions
# 
# # Downloaded TCGA BRCA peaks from https://gdc.cancer.gov/about-data/publications/ATACseq-AWG 
...
#                      c("#CCCCFF", "white", "red"))
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs-Heatmap_0_+_upgenes.pdf",width=4.15,height=4.25)
# ht <- ComplexHeatmap::Heatmap(mat, 
#                               name = "beta",
#                               col = col_fun, 
#                               right_annotation = ha, 
...
# 
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs_0_+_upgenes.pdf",width=18,height=4)
# p1+p2+p3
# dev.off()
# 
# # Gene set ORA 
...
#                                             qvalueCutoff=1)
# 
# saveRDS(em.p2g.up.gene,"./Luminal_Cohort_Results-updates-Revised-updated/em_p2g_0_+_up_gene.rds")
# 
# enrichplot::dotplot(em.p2g.up.gene,showCategory=3)+
#   ggtitle(paste0(length(unique(p2g.up.gene$LME_cell_type_1_model_geneName))," unique genes"))
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_0_+_upgenes-DotPlot.pdf", 
#        width = 11,
#        height = 6)
# 
# barplot(em.p2g.up.gene,showCategory=3)+
#   ggtitle(paste0(length(unique(p2g.up.gene$LME_cell_type_1_model_geneName))," unique genes"))
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_0_+_upgenes-BarPlot_Count.pdf", 
#        width = 11,
#        height = 6)
# 
# mutate(em.p2g.up.gene, qscore = -log(p.adjust, base=10)) %>% 
#   barplot(x="qscore",showCategory=3)
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_0_+_upgenes-BarPlot_QScore.pdf", 
#        width = 11,
#        height = 6)
# 
# # Screen for up genes and peaks
# 
# # Get up genes in normal v. cancer
# daps <- readRDS(paste0("./Luminal_Cohort_Results-updates-Revised-updated/DAPs_padj_",padj_cutoff,"_log2FC_",log2fc_cutoff,".rds"))
# up.peaks <- dplyr::filter(daps, log2FoldChange <= -log2fc_cutoff)
# 
# p2g.up.gene.up.peak <- p2g[p2g$LME_cell_type_1_model_geneName %in% up.genes$gene &
#                              p2g$LME_cell_type_1_model_peakName %in% up.peaks$gene,]
# 
# saveRDS(p2g.up.gene.up.peak,"./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-0_+-upgenes_uppeaks.rds")
# 
# # Annotate ENCODE and TCGA regions
# 
# # Downloaded TCGA BRCA peaks from https://gdc.cancer.gov/about-data/publications/ATACseq-AWG 
...
#                      c("#CCCCFF", "white", "red"))
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs-Heatmap_0_+_upgenes_uppeaks.pdf",width=4.15,height=4.25)
# ht <- ComplexHeatmap::Heatmap(mat, 
#                               name = "beta",
#                               col = col_fun, 
#                               right_annotation = ha, 
...
# 
# 
# pdf("Luminal_Cohort_Results-updates-Revised-updated/Significant_Differential_P2Gs_0_+_upgenes_uppeaks.pdf",width=18,height=6)
# p1+p2+p3
# dev.off()
# 
# # Gene set ORA 
...
#                                                     qvalueCutoff=1)
# 
# saveRDS(em.p2g.up.gene.up.peak,"./Luminal_Cohort_Results-updates-Revised-updated/em_p2g_0_+_up_gene_up_peak.rds")
# 
# enrichplot::dotplot(em.p2g.up.gene.up.peak,showCategory=3)+
#   ggtitle(paste0(length(unique(p2g.up.gene.up.peak$LME_cell_type_1_model_geneName))," unique genes"))
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_0_+_upgenes_uppeaks-DotPlot.pdf", 
#        width = 11,
#        height = 6)
# 
# barplot(em.p2g.up.gene.up.peak,showCategory=3)+
#   ggtitle(paste0(length(unique(p2g.up.gene.up.peak$LME_cell_type_1_model_geneName))," unique genes"))
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_0_+_upgenes_uppeaks-BarPlot_Count.pdf", 
#        width = 11,
#        height = 6)
# 
# mutate(em.p2g.up.gene.up.peak, qscore = -log(p.adjust, base=10)) %>% 
#   barplot(x="qscore",showCategory=3)
# ggsave("./Luminal_Cohort_Results-updates-Revised-updated/GeneSet_ORA_Hallmark_MSigDB_0_+_upgenes_uppeaks-BarPlot_QScore.pdf", 
#        width = 11,
#        height = 6)
# 
...
plotP2Gs <- function(ArchRProj, 
  ...
  pdf(paste0(outpath,"P2G_Condition_and_Patient_BrowserTracks_ExpressionPlots_ScatterPlots-",
             unique(sub$geneName),"-", identifier,".pdf"),width=9,height=11)
  ...
  pdf(paste0(outpath,"P2G_Patient_BrowserTracks_ExpressionPlots_ScatterPlots-",
             unique(sub$geneName),"-", identifier,".pdf"),width=8.5,height=7)
  ...
  for( i in 1:length(sort(unique(sub$peakName))) ){
    ...
    pdf(paste0(outpath,"P2G_ScatterPlots-", 
               ...
               unique(sub$geneName),"-", identifier,"-Interaction.pdf"),width=10.1,height=2.25)
    print(PlotsList[[i]][[1]])
    dev.off()
    
    pdf(paste0(outpath,"P2G_ScatterPlots-", 
               sort(unique(sub$peakName))[i],"-",
               unique(sub$geneName),"-", identifier,"-Cancer.pdf"),width=8,height=2.25)
    print(PlotsList[[i]][[2]])
    dev.off()
    
    pdf(paste0(outpath,"P2G_ScatterPlots-", 
               sort(unique(sub$peakName))[i],"-",
               unique(sub$geneName),"-", identifier,"-Normal.pdf"),width=6.5,height=2.25)
    print(PlotsList[[i]][[3]])
    dev.off()
...
# Remove sample with low # of cells
samplesToUse <- names(table(proj$predictedGroup)[table(proj$predictedGroup) >150])
...
proj$Patient <- ifelse(proj$Sample %in% c("43E7CL","43E7BL"),"43E7BL_43E7CL",proj$Sample)
...
# Read in dds 
dds <- readRDS("./Luminal_Cohort_Results-updates-Revised-updated/pseudobulk_expression_dds.rds")
...
colData(dds)$Patient <- factor(colData(dds)$Patient,levels = c("1-35EE8L",
                                                                  "2-3821AL",
                                                                  "3-3C7D1L",
                                                                  "4-3FCDEL",
                                                                  "5-44F0AL",
...
p2g <- readRDS("./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-+_0-upgenes.rds")
...
p2g.crabp2 <- p2g[p2g$geneName == "CRABP2",]

# Read in cancerSpecificEnh 
cancerSpecificEnh <- readRDS("./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-+_0-upgenes.rds")
cancerSpecificEnh <- cancerSpecificEnh[cancerSpecificEnh$LME_cell_type_1_model_geneName == "CRABP2",]

# Read in cancerEnh and normalEnh 
allP2Gs <- readRDS("./Luminal_Cohort_Results-updates-Revised-updated/p2gs_univariate-postfilter.rds")
cancerEnh <- allP2Gs[allP2Gs$DiffClass %in% c("+/+","+/0","+/-") & allP2Gs$peakType %in% c("Intronic","Distal") & allP2Gs$LME_cell_type_1_model_geneName == "CRABP2",]
normalEnh <- allP2Gs[allP2Gs$DiffClass %in% c("+/+","0/+","-/+") & allP2Gs$peakType %in% c("Intronic","Distal") & allP2Gs$LME_cell_type_1_model_geneName == "CRABP2",]

# Establish colors for cancer and normal conditions, and patient colors based on factor leveling
cancerCol<- "#a84343"
normalCol<- "#828282"
patientCols <- c(rep(cancerCol,8),rep(normalCol,4))
patientScatterCols <- c(rep(normalCol,4),rep(cancerCol,8))

# Read in Luminal meta_cell_data 
meta_cell_data <- readRDS("./LME_Luminal_out-SingFits_OLS/meta_cell_data.rds")
...
outpath = "./Luminal_Cohort_Results-updates-Revised-updated/"
...
p2g.crabp2.high <- p2g.crabp2[p2g.crabp2$LME_cell_type_1_model_peak_effect_size == max(p2g.crabp2$LME_cell_type_1_model_peak_effect_size),]

# Plots for CRABP2 (highest effect size)
plotP2Gs(ArchRProj = proj, 
         p2g = p2g.crabp2.high,
         meta_cell_data = meta_cell_data,
         encode.gr = encode.gr,
...
p2g.elf3 <- p2g[p2g$geneName == "ELF3",]

# Read in cancerSpecificEnh 
cancerSpecificEnh <- readRDS("./Luminal_Cohort_Results-updates-Revised-updated/p2gs_interaction-+_0-upgenes.rds")
cancerSpecificEnh <- cancerSpecificEnh[cancerSpecificEnh$LME_cell_type_1_model_geneName == "ELF3",]

# Read in cancerEnh and normalEnh 
allP2Gs <- readRDS("./Luminal_Cohort_Results-updates-Revised-updated/p2gs_univariate-postfilter.rds")
cancerEnh <- allP2Gs[allP2Gs$DiffClass %in% c("+/+","+/0","+/-") & allP2Gs$peakType %in% c("Intronic","Distal") & allP2Gs$LME_cell_type_1_model_geneName == "ELF3",]
normalEnh <- allP2Gs[allP2Gs$DiffClass %in% c("+/+","0/+","-/+") & allP2Gs$peakType %in% c("Intronic","Distal") & allP2Gs$LME_cell_type_1_model_geneName == "ELF3",]
...
# Subset to highest effect size P2G
p2g.elf3.high <- p2g.elf3[p2g.elf3$LME_cell_type_1_model_peak_effect_size == max(p2g.elf3$LME_cell_type_1_model_peak_effect_size),]

# Plots for ELF3 (highest effect size)
plotP2Gs(ArchRProj = proj, 
         p2g = p2g.elf3.high,
         meta_cell_data = meta_cell_data,
         encode.gr = encode.gr,
