...
# Franco Lab
# Description: Merge Luminal_TN Samples and Re-cluster
################################################################################
library(Seurat)
library(tidyverse)
library(dplyr)
...
sub <- rna[,rna$RNA_snn_res.0.4 == "9"]
table(sub$orig.ident)

sub <- rna[,rna$RNA_snn_res.0.4 %in% c("20",
                                       "6",
                                       "23",
                                       "18",
                                       "12")]
...
if(dir.exists("Luminal_TN_Cohort_scRNA-TESTING")==TRUE){
  print("Directory Luminal_TN_Cohort_scRNA-TESTING already exists!")
}else{
  dir.create("Luminal_TN_Cohort_scRNA-TESTING")
}

DimPlot(rna,group.by = "SCSubtype")
ggsave(paste0("./Luminal_TN_Cohort_scRNA-TESTING/UMAP_SCSubtype_",
              ID,".pdf"),
       width = 8,
       height = 8)
DimPlot(rna,group.by = "orig.ident")
ggsave(paste0("./Luminal_TN_Cohort_scRNA-TESTING/UMAP_Sample_",
              ID,".pdf"),
       width = 8,
       height = 8)
DimPlot(rna,group.by = "RNA_snn_res.0.015")
ggsave(paste0("./Luminal_TN_Cohort_scRNA-TESTING/UMAP_Cluster_",
              ID,".pdf"),
       width = 8,
       height = 8)
# Begin plotting
...
df %>%
  ...
  theme_classic()
ggsave(paste0("./Luminal_TN_Cohort_scRNA-TESTING/SCSubtype_Proportion_BarChart_",
              ID,".pdf"),
       width = 6,
       height = 8)
...
df %>%
  ...
  coord_flip()+theme_classic()+xlab("Clusters")+ylab("Fraction of cells")+
  theme_classic()
ggsave(paste0("./Luminal_TN_Cohort_scRNA-TESTING/Cluster_Proportion_BarChart_",
              ID,"-Sample.pdf"),
       width = 6,
       height = 8)
