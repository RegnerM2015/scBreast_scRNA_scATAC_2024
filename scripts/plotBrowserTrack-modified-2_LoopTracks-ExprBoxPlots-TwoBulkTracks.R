...
plotBrowserTrack.mod.3 <- function(
  ...
  .validInput(input = seurat, name = "seurat", valid = c("Seurat"))
...
.bulkTracks <- function(
  ...
  p <- ggplot(df, aes_string("x","y", color = "group", fill = "group")) + 
    annotate("rect", xmin=(start(peakCoords)-2000), xmax=(end(peakCoords)+2000), ymin=0, ymax=Inf, alpha=0.4, fill="deepskyblue1")+
...
.featureTracks <- function(
  ...
  if(!is.null(features)){
    ...
    p <- ggplot(data = featureO, aes(color = name)) +
      ...
      theme_ArchR(baseSize = baseSize, baseLineSize = borderWidth, baseRectSize = borderWidth) +
      guides(color = "none", fill = "none") + theme(strip.text.y = element_text(size = facetbaseSize, angle = 0), strip.background = element_blank())+
      annotate("rect", xmin=(start(peakCoords)-2000), xmax=(end(peakCoords)+2000), ymin=0, ymax=Inf, alpha=0.25, fill="deepskyblue1")
...
.combinedFeaturePlot <- function(
  ...
  exprViolinPlot <- VlnPlot(seurat,features = geneSymbol,pt.size=0,group.by = groupBy)+
    geom_boxplot()+
    coord_flip()+
    NoLegend()
  ...
  exprViolinPlot <- VlnPlot(seurat,features = geneSymbol,pt.size=0,group.by = groupByTop)+
    geom_boxplot()+
    coord_flip()+
    NoLegend()
  df <- exprViolinPlot$data
