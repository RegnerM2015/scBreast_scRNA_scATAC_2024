#!/usr/bin/env bash
#SBATCH --job-name Submit-Individual_Samples_scRNA-MultiKClustering
#SBATCH --cpus-per-task 1
#SBATCH -c 1
#SBATCH --mem 4g
#SBATCH --partition allnodes
#SBATCH --output scripts/Submit-Individual_Samples_scRNA-MultiKClustering.out 
...
do
    jobfile=scripts/Individual_Samples_scRNA-MultiKClustering-${i}.sh
    cat <<EOF > $jobfile
#!/usr/bin/env bash
#SBATCH --job-name Individual_Samples_scRNA-MultiKClustering-${i}
#SBATCH -c 32
#SBATCH --mem 80g
#SBATCH --partition allnodes
#SBATCH --output scripts/Individual_Samples_scRNA-MultiKClustering-${i}.out 
...
singularity exec --bind /datastore \\
                 ...
                 R CMD BATCH '--args ${i}' scripts/Individual_Samples_scRNA-MultiKClustering.R scripts/Individual_Samples_scRNA-MultiKClustering-${i}.Rout
EOF
    chmod +x ${jobfile}
