...
#' @param logFile The path to a file to be used for logging ArchR output.
#' @export
plotBrowserTrack.mod.3 <- function(
  ArchRProj = NULL, 
  region = NULL, 
  groupBy = "Clusters",
  groupByTop = "Clusters",
  useGroups = NULL, 
  plotSummary = c("bulkTrackTop", "bulkTrack", "featureTrack", "loopTrack", "geneTrack"),
  sizes = c(10, 1.5, 3, 4),
  features = getPeakSet(ArchRProj),
  loops = getCoAccessibility(ArchRProj),
  loops.1 = getCoAccessibility(ArchRProj),
  loops.2 = getCoAccessibility(ArchRProj),
  geneSymbol = NULL,
  useMatrix = NULL,
  log2Norm = TRUE,
  upstream = 50000,
  ...
  .validInput(input = groupByTop, name = "groupByTop", valid = c("character", "null"))
  ...
  .validInput(input = features, name = "features", valid = c("granges", "grangeslist", "null"))
  .validInput(input = loops, name = "loops", valid = c("granges", "grangeslist", "null"))
  .validInput(input = loops.1, name = "loops.1", valid = c("granges", "grangeslist", "null"))
  .validInput(input = loops.2, name = "loops.2", valid = c("granges", "grangeslist", "null"))
  ...
  .validInput(input = palTop, name = "palTop", valid = c("palette", "null"))
  ...
  # .validInput(input = dds, name = "dds", valid = c("DESeqDataSet","null"))
  
  tstart <- Sys.time()
  .startLogging(logFile=logFile)
  .logThis(mget(names(formals()),sys.frame(sys.nframe())), "plotBrowserTrack Input-Parameters", logFile = logFile)
  
  print(tolower(plotSummary))
  ##########################################################
  # Get Region Where Plot Will Occur (GenomicRanges)
  ##########################################################
  .logDiffTime(paste0("Validating Region to plot for ",tolower(plotSummary)), t1=tstart, verbose=verbose, logFile=logFile)
  if(is.null(region)){
  ...
  ggList <- lapply(seq_along(region), function(x){
    ...
    if("featuretrack" %in% tolower(plotSummary)){
      if(!is.null(features)){
        ...
        plotList$featuretrack <- .featureTracks(
          ...
          logFile = logFile,
          highlightDistance = highlightDistance,
          peakCoords = peakCoords) + theme(plot.margin = unit(c(0.1, 0.75, 0.1, 0.75), "cm"))
      }
    }
    ...
    if("looptrack2" %in% tolower(plotSummary)){
      if(!is.null(loops.2)){
        .logDiffTime(sprintf("Adding Loop Tracks (%s of %s)",x,length(region)), t1=tstart, verbose=verbose, logFile=logFile)
        plotList$looptrack2 <- .loopTracks(
          loops = loops.2, 
          region = region[x], 
          facetbaseSize = facetbaseSize,
          hideX = TRUE, 
...
.bulkTracks <- function(
  ...
  p <- ggplot(df, aes_string("x","y", color = "group", fill = "group")) + 
    annotate("rect", xmin=(start(peakCoords)-highlightDistance), xmax=(end(peakCoords)+highlightDistance), ymin=0, ymax=Inf, alpha=0.4, fill="deepskyblue1")+
...
.featureTracks <- function(
  ...
  if(!is.null(features)){
    ...
    if(is.null(pal)){
      # pal <- paletteDiscrete(set = "paired", values = rev(unique(paste0(featureO$name))))
      pal <- rep("black",length(rev(unique(paste0(featureO$name)))))
    }
    
    # featureO$name <- factor(paste0(featureO$name), levels=names(featureList))
    
    # rect.df <- as.data.frame(peakCoords)
    # rect.df$xmin <- rect.df$start-1000
    # rect.df$xmax <- rect.df$end+1000
    # 
    # p <- ggplot(data = featureO, aes(color = name)) +
    #   geom_rect(data=rect.df,aes(xmin=xmin, xmax=xmax, fill="deepskyblue1"),  ymin=0, ymax=Inf, alpha=0.15)+
    ...
    p <- ggplot(data = featureO, aes(color = name)) +
      ...
      theme_ArchR(baseSize = baseSize, baseLineSize = borderWidth, baseRectSize = borderWidth) +
      guides(color = "none", fill = "none") + theme(strip.text.y = element_text(size = facetbaseSize, angle = 0), strip.background = element_blank())+
      annotate("rect", xmin=(start(peakCoords)-highlightDistance), xmax=(end(peakCoords)+highlightDistance), ymin=0, ymax=Inf, alpha=0.25, fill="deepskyblue1")
  ...
  if(!is.null(loops)){
    ...
    if(testDim){
      ...
      if(is.null(pal)){
        ...
        pal <- colorRampPalette(c("blue", "#EEEEEE", "red"))(100)
      }
      ...
      p <- ggplot(data = data.frame(loopO), aes(x = x, y = y, group = id, color = value)) + 
        ...
        theme(legend.text = element_text(size = baseSize)) +
        theme_ArchR(baseSize = baseSize, baseLineSize = borderWidth, baseRectSize = borderWidth, legendPosition = "left") +
...
.combinedFeaturePlot <- function(
  ...
  sig_df <- data.frame(normCounts[rownames(normCounts) %in% c(geneSymbol),])
  sig_df$cluster_sample_id <- rownames(sig_df)
  
  ## Join counts data frame with metadata
  sig_df <- plyr::join(sig_df, as.data.frame(colData(dds)),
                             by = "cluster_sample_id")
  colnames(sig_df)[1] <- "value"
  
  # set.seed(seed)
  featurePlot <- ggplot(sig_df, aes(y = groupBy, x = log10(value+1), color = .data[[groupBy]])) +
    geom_point(position = position_jitter(seed = seed, width=0,height=0.5), size= 1.25)+
    theme_bw()+NoLegend()+
    facet_wrap(~.data[[groupBy]], ncol=1,scales="free_y",strip.position="right") +
    scale_color_manual(values=pal)+
    ...
    ggtitle(paste0("Pseudobulk scRNA expr: ",geneSymbol))
    
  # exprViolinPlot <- VlnPlot(dds,features = geneSymbol,pt.size=0,group.by = groupBy)+
  #   geom_boxplot()+
  #   coord_flip()+
  #   NoLegend()
  # df <- exprViolinPlot$data
  # colnames(df) <- c("gene","ident")
  # df$ident <- factor(df$ident,levels=gtools::mixedsort(unique(paste(df$ident))))
  # featurePlot <- ggplot(df,aes(y=ident,x=gene))+
  #   geom_boxplot(aes(fill=ident),lwd=0.45,outlier.size = 0.25,fatten = 0.95)+NoLegend()+
  #   theme_classic()+NoLegend()+xlab(paste0(geneSymbol," expression in scRNA-seq"))+
  #   scale_fill_manual(values=pal)+
  #   facet_wrap(~ident, ncol=1,scales="free_y",strip.position="right") +
  #   guides(fill = "none", colour = "none") +
    # theme_ArchR(baseSize = baseSize,
    #             baseRectSize = borderWidth,
    #             baseLineSize = tickWidth,
    ...
    # theme(plot.margin = unit(c(0.35, 0.15, 0.35, 0.15), "cm"),
    #       plot.title = element_text(size = 8))+
    # ggtitle(paste0("scRNA-seq expression: ",geneSymbol))
    # 
    # 
  # second bulktrack on top
  
  # set.seed(seed)
  featurePlotTop <- ggplot(sig_df, aes(y = .data[[groupByTop]], x = log10(value+1), color = .data[[groupByTop]])) +
    geom_point(position = position_jitter(seed = seed, width=0,height=0.5), size= 1.25)+
    theme_bw()+NoLegend()+
    facet_wrap(~.data[[groupByTop]], ncol=1,scales="free_y",strip.position="right") +
    scale_color_manual(values=palTop)+
    guides(fill = "none", colour = "none") +
    theme(panel.spacing= unit(0, "lines"),
          axis.title.x=element_blank(),
          axis.title.y=element_blank(),
  ...
  # exprViolinPlot <- VlnPlot(dds,features = geneSymbol,pt.size=0,group.by = groupByTop)+
  #   geom_boxplot()+
  #   coord_flip()+
  #   NoLegend()
  # df <- exprViolinPlot$data
  # colnames(df) <- c("gene","ident")
  # df$ident <- factor(df$ident,levels=gtools::mixedsort(unique(paste(df$ident))))
  # featurePlotTop <- ggplot(df,aes(y=ident,x=gene))+
  #   geom_boxplot(aes(fill=ident),lwd=0.45,outlier.size = 0.25,fatten = 0.95)+NoLegend()+
  #   theme_classic()+NoLegend()+xlab(paste0(geneSymbol," expression in scRNA-seq"))+
  #   scale_fill_manual(values=palTop)+
  #   facet_wrap(~ident, ncol=1,scales="free_y",strip.position="right") +
  #   guides(fill = "none", colour = "none") +
    # theme_ArchR(baseSize = baseSize,
    #             baseRectSize = borderWidth,
  ...
  print(tolower(names(plotList)))
  
  if("bulktracktop" %in% tolower(names(plotList))){
    idx <- which(tolower(names(plotList)) == "bulktracktop")
    
    p <- plotList[[idx]] + featurePlotTop + plot_spacer()
    
    plotList[idx] <- NULL
    
    idx <- which(tolower(names(plotList)) == "bulktrack")
    
    p <- p + plotList[[idx]] + featurePlot + plot_spacer()
